...
```bash
cd /home/dzoan/OSPC2FLEX/Openstack-SisandBrotherInArms/arm-openstack-poc
...
```bash
find arm-openstack-poc/scripts -type f -name "*.sh" -print0 | xargs -0 -I{} bash -n {}
python3 -m py_compile arm-openstack-poc/scripts/02_generate_inventory.py
chmod +x arm-openstack-poc/scripts/*.sh
```
